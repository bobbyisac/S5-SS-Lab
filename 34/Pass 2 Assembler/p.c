...
{
	FILE *f1,*f2,*f3,*f4;
	int lc,sa,l,op1,o,len,opcode=10,count=0;
	...
	while(!feof(f1))
	{
		...
		if(strcmp(m1,otp)!=0)
		{
			...
			else if(strcmp(m1,"BYTE")==0 || strcmp(m1,"RESW")==0 || strcmp(m1,"RESB")==0)
			{
				fprintf(f2,"%s\t%s\t%s\t-\n",z,m1,op);
			}
		}
	...
	f2=fopen("text.txt","w");
	f3=fopen("inter.txt","r");
	fscanf(f3,"%s%s%s%s",z,la,m1,op);
	fprintf(f2,"H^%s^%d^%d\nT",la,sa,lc-sa);
